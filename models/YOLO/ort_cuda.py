...
class Model(Model):
    """YOLO inference with using CUDA Execution Provider"""

    def __init__(self):
        super().__init__()
        self.sess = None
        self.sess_data = {'providers': ['CUDAExecutionProvider']}
        if self.sess_data['providers'][0] not in ort.get_available_providers():
            raise Exception('CUDA Execution Provider is not available')
