...
class TestModelBatchSize:
    ...
    def check_batch_output(self, out, x):
        """
        ...
        # '{ "Running Batch": X },'
        assert f'{{ "Running Batch": {x} }},' in out, f'Expected \'{{ "Running Batch": {x} }},\' in output'
        # '{ "Model Warm Up X" : "<float>" },' (any float)
        warm_up_pattern = re.compile(re.escape(f'{{ "Model Warm Up {x}"') + r'\s*:\s*"[^"]+"\s*\},?')
        assert warm_up_pattern.search(out), f'Expected line like \'{{ "Model Warm Up {x}" : "<float>" }},\' in output'
        # '{ "Total Inference X" : "<float>" },'
        total_inf_pattern = re.compile(re.escape(f'{{ "Total Inference {x}"') + r'\s*:\s*"[^"]+"\s*\},?')
        assert total_inf_pattern.search(out), (
            f'Expected line like \'{{ "Total Inference {x}" : "<float>" }},\' in output'
        )
        # '"Batch Size" : "X",'
        assert f'"Batch Size" : {x},' in out, f'Expected \'"Batch Size" : {x},\' in output'

    @pytest.mark.parametrize('batchs', [[1], [1, 2], [1, 2, 4, 8, 16, 32, 64, 128]])
    def test_batch_output_lines(self, batchs):
        """
        Run test_perf.py with selftest.models.noisy_model and batchs --batch-size values.
        """
        # Use few runs per batch to keep test fast
        stdout, stderr, code = self.run_subprocess(['--runs', '2', '--batch-size', ','.join(str(b) for b in batchs)])
        out = get_combined_output(stdout, stderr)
        assert code == 0, f'Subprocess failed:\nstdout:\n{stdout}\nstderr:\n{stderr}'

        for x in batchs:
            self.check_batch_output(out, x)
