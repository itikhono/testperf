...
def performance_report(model, model_name, read_times, inference_times, warm_up_times, batches):
    ...
    try:
        ...
        read_sheet[col_letter + str(offset_stat_row + 0)] = (
            '=AVERAGE(' + col_letter + str(offset_row) + ':' + col_letter + str(last_row) + ')'
        )
        read_sheet[col_letter + str(offset_stat_row + 1)] = (
            '=MEDIAN(' + col_letter + str(offset_row) + ':' + col_letter + str(last_row) + ')'
        )
        read_sheet[col_letter + str(offset_stat_row + 2)] = (
            '=_xlfn.PERCENTILE.INC(' + col_letter + str(offset_row) + ':' + col_letter + str(last_row) + ', 0.9)'
        )
        read_sheet[col_letter + str(offset_stat_row + 3)] = (
            '=_xlfn.PERCENTILE.INC(' + col_letter + str(offset_row) + ':' + col_letter + str(last_row) + ', 0.95)'
        )
        read_sheet[col_letter + str(offset_stat_row + 4)] = (
            '=_xlfn.PERCENTILE.INC(' + col_letter + str(offset_row) + ':' + col_letter + str(last_row) + ', 0.99)'
        )
        read_sheet[col_letter + str(offset_stat_row + 5)] = (
            '=MIN(' + col_letter + str(offset_row) + ':' + col_letter + str(last_row) + ')'
        )
        read_sheet[col_letter + str(offset_stat_row + 6)] = (
            '=MAX(' + col_letter + str(offset_row) + ':' + col_letter + str(last_row) + ')'
        )
        ...
        for batch_index in range(len(batches)):
            ...
            inference_sheet[col_letter + str(offset_stat_row + 0)] = (
                '=AVERAGE(' + col_letter + str(offset_row) + ':' + col_letter + str(last_row) + ')'
            )
            inference_sheet[col_letter + str(offset_stat_row + 1)] = (
                '=MEDIAN(' + col_letter + str(offset_row) + ':' + col_letter + str(last_row) + ')'
            )
            inference_sheet[col_letter + str(offset_stat_row + 2)] = (
                '=_xlfn.PERCENTILE.INC(' + col_letter + str(offset_row) + ':' + col_letter + str(last_row) + ', 0.9)'
            )
            inference_sheet[col_letter + str(offset_stat_row + 3)] = (
                '=_xlfn.PERCENTILE.INC(' + col_letter + str(offset_row) + ':' + col_letter + str(last_row) + ', 0.95)'
            )
            inference_sheet[col_letter + str(offset_stat_row + 4)] = (
                '=_xlfn.PERCENTILE.INC(' + col_letter + str(offset_row) + ':' + col_letter + str(last_row) + ', 0.99)'
            )
            inference_sheet[col_letter + str(offset_stat_row + 5)] = (
                '=MIN(' + col_letter + str(offset_row) + ':' + col_letter + str(last_row) + ')'
            )
            inference_sheet[col_letter + str(offset_stat_row + 6)] = (
                '=MAX(' + col_letter + str(offset_row) + ':' + col_letter + str(last_row) + ')'
            )
            ...
            inference_sheet[col_letter + str(offset_stat_row + 12)] = (
                '=' + str(batches[batch_index]) + ' * ' + col_letter + str(offset_stat_row + 7)
            )
            inference_sheet[col_letter + str(offset_stat_row + 13)] = (
                '=' + str(batches[batch_index]) + ' * ' + col_letter + str(offset_stat_row + 8)
            )
            inference_sheet[col_letter + str(offset_stat_row + 14)] = (
                '=' + str(batches[batch_index]) + ' * ' + col_letter + str(offset_stat_row + 9)
            )
            inference_sheet[col_letter + str(offset_stat_row + 15)] = (
                '=' + str(batches[batch_index]) + ' * ' + col_letter + str(offset_stat_row + 10)
            )
            inference_sheet[col_letter + str(offset_stat_row + 16)] = (
                '=' + str(batches[batch_index]) + ' * ' + col_letter + str(offset_stat_row + 11)
            )
        ...
        for idx in range(1, max_rows + 1):
            ...
            for batch_index in range(len(batches)):
                row.append(
                    inference_table[batch_index][idx - 1] if len(inference_table[batch_index]) > idx - 1 else None
                )
        ...
        for batch_index in range(len(batches)):
            series = Series(
                values=Reference(
                    inference_sheet,
                    min_col=batch_index + offset_col,
                    min_row=offset_row,
                    max_col=batch_index + offset_col,
                    max_row=last_row,
                ),
                title=f'Batch {batches[batch_index]}',
            )
            chart.series.append(series)
        chart.width = 15
        chart.legend.position = 'b'
...
def list_loaded_modules():
    osname = platform.system()
    result = {'pid': os.getpid(), 'executable': sys.executable, 'modules': []}
...
def _windows_list_modules():
    ...
    try:
        ...
        for dll in process.memory_maps():
            modules.append({'name': os.path.basename(dll.path), 'path': dll.path})
        return modules
    except ImportError:
        pass
    except Exception:
    ...
    try:
        ...
        for mmap in process.memory_maps():
            if mmap.path and mmap.path not in seen_paths:
                ...
                modules.append({'name': os.path.basename(mmap.path), 'path': mmap.path})
        return modules
    except ImportError:
        pass
    except Exception:
                    ...
                    for part in parts:
                        if part.startswith('/') and os.path.exists(part):
                            if part not in seen_paths:
                                seen_paths.add(part)
                                modules.append({'name': os.path.basename(part), 'path': part})
    except Exception as e:
        modules.append({'error': str(e)})
